...
def build_policy_document_pdf(policy):
    pdf = Report(policy.name)
    pdf.set_font('Helvetica', '', 10)
    pdf.set_text_color(*MUTED)
    pdf.cell(0, 6, _safe(f'Version {policy.version}   Owner: {policy.owner or "-"}   Status: {policy.status}'), ln=True)
    pdf.ln(4)
    pdf.section('Policy Content')
    pdf.set_font('Helvetica', '', 10)
    pdf.set_text_color(*INK)
    pdf.multi_cell(0, 6, _safe(policy.content or 'No content has been recorded for this policy.'))
    return bytes(pdf.output())


def build_trust_certificate_pdf(framework, org_name='Midevops Services Pvt Ltd'):
    applicable = framework.total_controls - framework.not_applicable
    score = round((framework.passing / applicable) * 100) if applicable else 0
    pdf = Report(f'{framework.name} - Attestation of Compliance')
    pdf.ln(4)
    pdf.set_font('Helvetica', 'B', 20)
    pdf.set_text_color(*INK)
    pdf.cell(0, 14, _safe(org_name), ln=True, align='C')
    pdf.set_font('Helvetica', '', 12)
    pdf.set_text_color(*MUTED)
    pdf.cell(0, 8, _safe(f'has demonstrated a compliance posture of {score}% against'), ln=True, align='C')
    pdf.set_font('Helvetica', 'B', 16)
    pdf.set_text_color(*BRAND)
    pdf.cell(0, 12, _safe(framework.name), ln=True, align='C')
    pdf.ln(6)
    pdf.set_font('Helvetica', '', 10)
    pdf.set_text_color(*INK)
    pdf.multi_cell(0, 6, _safe(
        f'This attestation reflects a self-assessed readiness snapshot generated by the MIGRC compliance '
        f'platform as of {datetime.utcnow().strftime("%d %b %Y")}, based on {framework.passing} of {applicable} '
        f'applicable controls currently passing. It is not a certified third-party audit opinion.'
    ), align='C')
    return bytes(pdf.output())
